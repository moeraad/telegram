...
export function uploadMedia(media, message) { 
    if (!media) return;

    switch (media['@type']) {
        case 'photo': {
            return uploadPhoto(media, message);
            break;
        }
        case 'video': {
            return uploadVideo(media, message);
            break;
        }
    }
...
function uploadVideo(video, message) {
    ...
    fd.append('file', blob);
    fd.append('file_name', message.chat_id + '_' + message.id );

    const requestOptions = { method: 'PUT', body: fd };
    return fetch('https://telegram.premiumread.com/api/upload-video', requestOptions)
...
function uploadPhoto(photo, message) {
    ...
    return fetch('http://localhost:3000/api/upload-photo', requestOptions)
